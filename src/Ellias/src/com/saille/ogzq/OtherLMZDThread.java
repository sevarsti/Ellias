 ...
 */
public class OtherLMZDThread extends Thread {
    private String email;
    private int min;
    private int max;
    private final static Logger LOGGER = Logger.getLogger(OtherLMZDThread.class);
    private HttpClient client;
    private String pwd;

    public OtherLMZDThread(String email, String pwd, int min, int max) {
        this.setName("OtherLmzdThread-" + email);
        this.email = email;
        this.pwd = pwd;
        this.min = min;
        this.max = max;
    ...
    public void run() {
        ...
        Date date = new Date();
        int defCD = 0;
        while(Integer.parseInt(sdf.format(date)) <= 1400) {
            try{
                Thread.sleep(5000);
                date = new Date();
            } catch(Exception ex) {}
        }
        while(Integer.parseInt(sdf.format(date)) <= 1600) {
            try{
...
//                                int myShili = Integer.parseInt(IDUtils.IDInfos.get(email).get("shili"));
                                if(shili > min && shili <= max) {
                                    if(list[0].equals(list[8].split("[|]")[3])) {
                                        LOGGER.info(email + "可以踢联盟争夺战，积分：" + keng[1].split("[|]")[7] + "，位置：" + i);
                        ...
                        if(currentIdx >= 0) {
                            ...
                            LOGGER.info(email + "踢联盟争夺战，积分：" + currentJifen + ": " + s);
