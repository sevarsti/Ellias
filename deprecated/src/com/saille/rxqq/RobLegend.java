...
public class RobLegend {
    ...
    private static void loadPlayers() {
        try {
            File f = new File("D:\\work\\Ellias\\exploded\\rxqq\\robplayer.txt");
            ...
            while((tmp = br.readLine()) != null) {
                ...
                for(int i = 1; i <= 10; i++) {
                    if(StringUtils.isNotEmpty(prices[i])) {
                        RobLegend.prices.put(name + "_" + i, Integer.valueOf(Integer.parseInt(prices[i])));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String[] l1Params = RxqqUtils.getParams(new String(getMethod.getResponseBody(), getMethod.getResponseCharSet()));
            ...
            url = "http://login.sina.com.cn/sso/login.php?entry=wanwan&service=webgame03603&returntype=TEXT&callback=func";
            ...
            url = "http://wanwan.games.sina.com.cn/wanloginreg/wanlogin_fd/dogame.php?service=webgame03603";
            getMethod = new GetMethod(url);
            getMethod.addRequestHeader("Cookie", genCookie(cookies));
            ...
            int pageCount = 1;
            int maxPage = 1;
            String s = "http://s3.qiuqiu.2010.sina.com.cn/Auction.do?action=Select&ItemType=4&Order=2&ItemLevel=1&Index=2&PageIndex=%s";
            int errorCount = 0;
            int checkCount = 0;
            while(errorCount < 100) {
                System.out.println(new Date() + "循环..." + checkCount++);
                try {
    ...
    private static boolean checkRob(JSONObject player) {
        try {
            ...
            Integer setPrice = (Integer) prices.get(name + "_" + level);
            if(setPrice == null) {
                return false;
            }
            if(setPrice.intValue() >= price) {
